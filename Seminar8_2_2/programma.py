...
db = SqliteDatabase('E:\Programming\Visual_Studio_Code\Python_seminars_replay\Seminar8_2_2\gitudata.db')
...
class name_nama(BaseModel):
    # id = PrimaryKeyField(unique=True)
    name = TextField()  # тут мы будем хранить текст, поэтому указываем его тип как симвотльное поле
    # telefon = CharField()
# TextField()
#CharField()
    class Meta:
        db_table = 'name_nama' # та таблица которая нужна нам    

class department(BaseModel):
    # id = PrimaryKeyField(unique=True)   
    department_name = CharField()
    name_id = ForeignKeyField(name_nama) # ссылка на всю запись - id = PrimaryKeyField(unique=True)в верхней модели
...
class post(BaseModel):
    # id = PrimaryKeyField(unique=True)   
    post_name = CharField()
    department_id = ForeignKeyField(name_nama) # ссылка на всю запись
